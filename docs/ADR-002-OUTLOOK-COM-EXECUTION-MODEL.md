...
> **Revision 2026-09-04 (supersedes the 2026-09-03 revision)** — The retention decision has been
> overruled by the product owner: "hier geht es nur um Outlook". `ComInterop/Session/*` has been
> **deleted** in full, along with the CLI `session` command branch, the service session RPC
> handlers, the `[NoSession]` attribute, and the `Microsoft.Office.Interop.PowerPoint` reference.
> This restores the ADR's original position. The central decision is unchanged — Outlook executes
> via `OutlookDispatcher` — and the naming debt tracked by #12 is resolved by deletion.
> See [Fate of `ComInterop/Session/*`](#fate-of-cominteropsession).

> **Revision 2026-09-03 (superseded)** — This ADR originally stated that `ComInterop/Session/*`
> should be deleted wholesale once #26 removes the PowerPoint command surface. That directly
> contradicted #26's own acceptance criteria, which required the layer to be **retained**. The
> conflict was resolved in favour of #26 at the time; that resolution has since been reversed.
...
**Deleted.** The layer is gone as of the 2026-09-04 revision. `PptSession`, `PptBatch`,
`PptContext`, `IPptBatch`, `SessionManager`, `PptShutdownService` and `ResiliencePipelines` were
removed along with their 8 integration test files, the CLI `session` command branch, the service
session RPC handlers, and the `[NoSession]` attribute (every Outlook interface carried it, so the
generated session branches were dead code).

The 2026-09-03 revision kept the layer as dormant infrastructure on the grounds that it was mature,
tested COM plumbing that might serve a future third COM product. That bet was not taken. The
carrying cost turned out to be real rather than theoretical:

- Its integration tests launched PowerPoint on any unfiltered `dotnet test`, silently, on a
  developer's machine (#70).
- It kept `Microsoft.Office.Interop.PowerPoint` on the dependency graph, the dependency-review
  allow-list, and the CodeQL suppression list.
- It required permanent "this is dormant, do not use it" caveats in the contributor
  instructions, the READMEs, the testing guide, and the timeout guide — caveats that are load
  bearing precisely because the code looks usable.
- It anchored the `Ppt*` naming debt (#12) that could not be resolved either way while the code
  existed.

`OleMessageFilter` is the one piece of that infrastructure that was genuinely product-neutral. It
survives as a shared `ComInterop` utility, along with `OutlookDispatcher`, `ComUtilities` and
`FileAccessValidator`.

Anything the deleted layer taught us is recoverable from git history if a third COM product ever
justifies it.

## Naming Plan

- Outlook dispatcher type: `OutlookMcp.ComInterop.OutlookDispatcher`, at the root of the
  `OutlookMcp.ComInterop` project (the `Session/` folder no longer exists).
  `OutlookInteropRunner` in `OutlookMcp.Core` is a thin caller of the dispatcher rather than
  owning thread lifecycle itself.
- No `Ppt*`-prefixed types remain in the product. `PptToolsBase` was renamed to
  `OutlookToolsBase`. This resolves the naming debt tracked by #12.
...
|---|---|---|
| 1. Single long-lived STA dispatcher, filter registered once | ✅ Done | `ComInterop/OutlookDispatcher.cs` |
...
- **Generalize `ComInterop/Session/*` into an abstract multi-app session layer shared by both
  products.** Rejected: the abstraction would need to model "opens a document" vs. "attaches to a
  singleton" as two fundamentally different resource lifecycles, which is a larger refactor than
  the Outlook correctness work justifies. Moot as of the 2026-09-04 revision, since the layer has
  been deleted. Revisit from git history if a genuine third COM consumer appears — not for
  Outlook, which uses `OutlookDispatcher`.
- **Retain `ComInterop/Session/*` as dormant infrastructure.** Adopted in the 2026-09-03 revision,
  then reversed. The reconstruction-cost argument was real but outweighed by the carrying cost:
  a stray `dotnet test` launched PowerPoint (#70), the PowerPoint interop stayed on the dependency
  graph, and every contributor-facing document needed a "do not use this" caveat. See
  [Fate of `ComInterop/Session/*`](#fate-of-cominteropsession).

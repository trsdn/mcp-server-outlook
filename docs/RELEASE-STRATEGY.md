# OutlookMcp Release Strategy

This document outlines the unified release process for all OutlookMcp components.

## Overview

All OutlookMcp components are released together with a single version tag:
...
2. **MCP Server** → Unified NuGet (`OutlookMcp.McpServer` — includes CLI) + ZIP
...
|----------|--------|--------------|
| `OutlookMcp-MCP-Server-{version}-windows.zip` | ZIP | GitHub Release |
| `OutlookMcp-{version}.vsix` | VSIX | GitHub Release + VS Code Marketplace (~68-70 MB, self-contained) |
| `outlook-mcp-{version}.mcpb` | MCPB | GitHub Release |
| `outlook-skills-v{version}.zip` | ZIP | GitHub Release |
| `OutlookMcp.McpServer.{version}.nupkg` | NuGet | NuGet.org (unified — includes CLI) |

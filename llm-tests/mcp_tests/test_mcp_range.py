...
@pytest.mark.asyncio
async def test_mcp_range_set_get(aitest_run, outlook_mcp_server, outlook_mcp_skill, fixtures_dir):
    agent = Agent(
        name="mcp-range",
...
@pytest.mark.asyncio
async def test_mcp_range_error_handling(aitest_run, outlook_mcp_server, outlook_mcp_skill):
    agent = Agent(

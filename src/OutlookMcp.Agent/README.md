...
Official source-side Copilot SDK orchestrator for multi-phase PowerPoint deck generation through `mcp-server-outlook`.

## Purpose

`src\OutlookMcp.Agent` is the repository's first-class deck-building controller.
...
- `src\OutlookMcp.McpServer` stays focused on primitive PowerPoint capabilities
- `skills\shared\*.md` stays focused on LLM guidance
- `src\OutlookMcp.Agent` owns orchestration, retries, artifact validation, and run summaries

## Architectural Boundary

`OutlookMcp.Agent` is **not** a third server surface and it does **not** move orchestration into the MCP server.
...
--overwrite
--skip-verify
--mcp-server "C:\path\to\OutlookMcp.McpServer.exe"

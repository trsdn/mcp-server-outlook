...
function printHelp() {
  console.log(`outlook-mcp-agent

Official source-side Copilot SDK client for orchestrating mcp-server-outlook.

Usage:
  outlook-mcp-agent run --task "Build a 5-slide deck on ..." [options]
...
async function main() {
  ...
  try {
    ...
    if (error && error.code === "ERR_MODULE_NOT_FOUND") {
      throw new Error(
        "Missing runtime dependency. Run 'npm install' in src\\OutlookMcp.Agent before using the agent."
      );
    }
...
main().catch((error) => {
  console.error(`outlook-mcp-agent failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
});

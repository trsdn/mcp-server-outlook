...
function defaultOutputPath() {
  ...
  return resolve(process.cwd(), `outlook-mcp-agent-${stamp}.pptx`);
}
...
function loadArchetypeIds() {
  const archetypesDir = join(REPO_ROOT, "src", "OutlookMcp.Core", "Data", "archetypes");
...
function loadPlanningGuidance() {
  const registry = readText("src\\OutlookMcp.Core\\Data\\archetypes\\registry.md");
...
function buildExecutionPrompt({ task, plan, outputPath, showPowerPoint }) {
  ...
  return [
    ...
    "You are operating through mcp-server-outlook only.",
    "Do not rely on MCP batch execution or subagents.",
...
function buildRepairPrompt({ task, plan, outputPath, validationError, showPowerPoint }) {
  ...
  return [
    ...
    "Repair the presentation through mcp-server-outlook only.",
    "Do not rely on MCP batch execution or subagents.",
    "",
    "Repair goal:",

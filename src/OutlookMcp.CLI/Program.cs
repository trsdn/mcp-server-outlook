...
using OutlookMcp.CLI.Generated;
using OutlookMcp.CLI.Infrastructure;
using Spectre.Console;
using Spectre.Console.Cli;

namespace OutlookMcp.CLI;
...
internal sealed class Program
{
        ...
        app.Configure(config =>
        {
            ...
            config.AddBranch("service", branch =>
            {
                ...
                branch.AddCommand<ServiceStartCommand>("start")
                    .WithDescription("Start the migration service if not already running.");
                branch.AddCommand<ServiceStopCommand>("stop")
                    .WithDescription("Gracefully stop the migration service.");
                branch.AddCommand<ServiceStatusCommand>("status")
                    .WithDescription("Show service status (running, PID, legacy sessions, uptime).");
            });
    ...
    private static async Task<int> HandleVersionAsync()
    {
        ...
        if (updateAvailable)
        {
            ...
            AnsiConsole.MarkupLine($"[cyan]Run:[/] [white]dotnet tool update --global OutlookMcp.CLI[/]");
            AnsiConsole.MarkupLine($"[cyan]Release notes:[/] [blue]https://github.com/trsdn/mcp-server-outlook/releases/latest[/]");
        }
    ...
    private static int RunServiceDaemon(string? pipeNameOverride = null)
    {
        ...
        var service = new Service.OutlookMcpService();

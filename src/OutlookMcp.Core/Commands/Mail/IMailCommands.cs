...
[McpTool("mail", Title = "Outlook Mail Operations", Destructive = true, Category = "mail",
    ...
    + "search's query is a free-text match over subject, sender, recipients and the full message body, applied "
    + "after the structured filters. searchMode picks which engine answers it. 'advancedSearch' is the default: "
    + "Outlook runs the search itself with substring matching and no scan limit, so it finds a term buried deep "
    + "in a long message arbitrarily far back in a large folder, and nothing is opened by this server. "
    + "'clientScan' opens every candidate item and checks it here - also substring matching, but it stops at a "
    + "safety limit, so a match past that limit is reported as no such mail. 'fullText' uses Outlook's content "
    + "index: no scan limit, but whole-word matching, so it finds 'foo' in 'a foo arrived' and not inside "
    + "'foobar'. Every search response reports searchEngine ('advancedSearch', 'clientScan' or 'contentIndex'), "
    + "because an empty result means different things depending on which engine produced it; if the engine asked "
    + "for could not serve the query - a wildcard the DASL dialect cannot escape, or a store that will not run "
    + "the search - searchEngine names the one that actually ran and message explains why. A search Outlook "
    + "started but did not finish in time comes back truncated with the matches found so far and a message "
    + "saying so; do not read it as complete. "
    + "Cached Exchange Mode (the usual setup) answers from a local replica of the mailbox: fast, but only as "
    + "current as the last sync, and a profile that caches only recent months does not hold older mail locally "
    + "at all - so 'older than this profile caches' is a real explanation for a miss, not proof the mail does "
    + "not exist. Online Mode goes to the server for every request: always current, noticeably slower. Indexing "
    + "is separate again: 'fullText' needs an indexed store and fails outright on one that is unindexed or still "
    + "catching up, whereas the default 'advancedSearch' does not depend on the index and still answers there. "
    ...
    + "Every list and search response reports projection: 'table' when the rows were read from an Outlook "
    + "rowset without opening any message (the default, and roughly twenty times faster), or 'item' when "
    + "each message had to be opened because the request needed a body - includeBodyPreview, or a query in "
    + "the default clientScan mode. hasAttachment is always present; attachmentCount only comes back on the "
    + "'item' projection, because a rowset knows whether a message has attachments but not how many. A "
    + "missing attachmentCount therefore never means 'no attachments' - read hasAttachment for that, and "
    + "use attachment.list or mail.read when the exact number matters. "

...
namespace OutlookMcp.Generators.Cli;
...
public class CliSettingsGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(context.CompilationProvider,
            static (spc, compilation) =>
            {
                ...
                foreach (var (_, registryName) in categories)
                {
                    var cmdCode = GenerateCommandClass(registryName);
    ...
    /// </summary>
    private static List<(string CliName, string RegistryName)> DiscoverCategories(Compilation compilation)
    {
        var result = new List<(string CliName, string RegistryName)>();
        ...
        foreach (var reference in compilation.References)
        {
            ...
            foreach (var type in GetAllTypes(assembly.GlobalNamespace))
            {
                ...
                var attr = type.GetAttributes().FirstOrDefault(a =>
                    ...
                    a.AttributeClass?.ContainingNamespace?.ToDisplayString() == "OutlookMcp.Core.Attributes");
                ...
                result.Add((cliName, categoryPascal));
            }
        }
    ...
    private static string GenerateCommandClass(string registryName)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine("using OutlookMcp.CLI.Infrastructure;");
        sb.AppendLine("using OutlookMcp.Generated;");
        sb.AppendLine();
        sb.AppendLine("namespace OutlookMcp.CLI.Generated;");
        sb.AppendLine();
    ...
    private static string GenerateRegistration(List<(string CliName, string RegistryName)> categories)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("#nullable enable");
        ...
        sb.AppendLine("    {");
        foreach (var (cliName, registryName) in categories.OrderBy(c => c.CliName))

...
using OutlookMcp.Generators.Common;

namespace OutlookMcp.Generators;
...
public class ServiceRegistryGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateServiceRegistry(ServiceInfo info)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("namespace OutlookMcp.Generated;");
        sb.AppendLine();
    ...
    private static void GenerateCliRouteMethod(StringBuilder sb, ServiceInfo info)
    {
        ...
        foreach (var method in info.Methods)
        {
            ...
            foreach (var p in method.Parameters.Where(p => !p.IsFileOrValue && (p.IsRequired || (!p.HasDefault && !p.TypeName.EndsWith("?"))) && StringHelper.IsStringType(p.TypeName)))
            {
                ...
                sb.AppendLine($"                    OutlookMcp.Core.Utilities.ParameterTransforms.RequireNotEmpty({paramName}, \"{paramName}\", \"{method.ActionName}\");");
            }
            ...
            {
                sb.AppendLine($"                    var resolved{StringHelper.ToPascalCase(p.Name)} = OutlookMcp.Core.Utilities.ParameterTransforms.ResolveFileOrValue({p.Name}, {p.Name}{p.FileSuffix});");
                // Validate required FileOrValue parameters AFTER resolution
                if (p.IsRequired || (!p.HasDefault && !p.TypeName.EndsWith("?")))
                {
                    sb.AppendLine($"                    OutlookMcp.Core.Utilities.ParameterTransforms.RequireNotEmpty(resolved{StringHelper.ToPascalCase(p.Name)}, \"{p.Name}\", \"{method.ActionName}\");");
                }
            }
    ...
    private static string GenerateSkillManifest(List<ServiceInfo> categories)
    {
        ...
        sb.AppendLine("/// Used by OutlookMcp.Build.Tasks to generate SKILL.md files from templates.");
        sb.AppendLine("/// </summary>");
    ...
    // OutlookMcp.Generators.Shared and included as source files
    ...
    private static string GenerateDispatchHelpers()
    {
        ...
        sb.AppendLine("        //   echo '[[\"value\",1]]' | outlookcli range set-values --values -");
        ...
        sb.AppendLine("                \" Tip: PowerShell strips double-quotes from native executable arguments. Use --values-file <path> to pass JSON from a file, or pipe JSON and use --values - (e.g., echo '[[...]]' | outlookcli ... --values -).\");");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>");
    ...
    /// replacing hand-written Handle*CommandAsync methods in OutlookMcpService.
    /// </summary>
    ...
    private static void GenerateDispatchCase(StringBuilder sb, ServiceInfo info, MethodInfo method)
    {
        ...
        if (method.HasProgressParameter)
            callArgs.Add("OutlookMcp.ComInterop.ProgressContext.Current");

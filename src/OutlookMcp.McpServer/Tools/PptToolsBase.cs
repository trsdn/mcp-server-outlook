...
public static class PptToolsBase
{
        ...
        Func<Exception, string?>? customHandler = null)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                Console.Error.WriteLine($"[OutlookMcp] COM Exception in {toolName}/{actionName}: HResult=0x{comEx.HResult:X8}, Message={comEx.Message}");
                if (ex.StackTrace != null)
                {
                    Console.Error.WriteLine($"[OutlookMcp] StackTrace: {ex.StackTrace[..Math.Min(500, ex.StackTrace.Length)]}");
                }
            }
            else if (ex.InnerException is System.Runtime.InteropServices.COMException innerComEx)
            {
                Console.Error.WriteLine($"[OutlookMcp] Inner COM Exception in {toolName}/{actionName}: HResult=0x{innerComEx.HResult:X8}, Message={innerComEx.Message}");
            }

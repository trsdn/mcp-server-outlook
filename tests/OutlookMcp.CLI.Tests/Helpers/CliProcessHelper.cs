...
namespace OutlookMcp.CLI.Tests.Helpers;

/// <summary>
/// Helper for running outlookcli as a subprocess and capturing output.
/// Used by integration tests that verify CLI behavior end-to-end.
/// </summary>
internal static class CliProcessHelper
{
    /// <summary>
    /// Gets the path to the outlookcli executable.
    ...
    public static string GetExePath()
    {
        ...
        var exePath = Path.Combine(testDir, "outlookcli.exe");

        if (!File.Exists(exePath))
        {
            throw new FileNotFoundException(
                $"outlookcli.exe not found at {exePath}. Ensure OutlookMcp.CLI is a project reference.");
        }
    ...
    /// <summary>
    /// Runs an outlookcli command and captures the result.
    /// Always uses -q (quiet) mode for clean JSON output.
    /// </summary>
    /// <param name="args">Arguments to pass to outlookcli (e.g., "diag ping")</param>
    ...
    public static async Task<CliResult> RunAsync(string args, int timeoutMs = 30000, Dictionary<string, string>? environmentVariables = null)
    {
        ...
        if (!completed)
        {
            ...
            throw new TimeoutException($"outlookcli timed out after {timeoutMs}ms. Args: {args}");
        }
    ...
    /// <summary>
    /// Runs an outlookcli command and parses the JSON output.
    /// </summary>
...
/// <summary>
/// Result of running outlookcli as a subprocess.
/// </summary>

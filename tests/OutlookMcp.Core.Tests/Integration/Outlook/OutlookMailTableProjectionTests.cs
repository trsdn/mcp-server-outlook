...
public class OutlookMailTableProjectionTests(ITestOutputHelper output)
{
    ...
    ///
    /// <para>
    /// This used to discard the result. That is not a tidiness problem: when Outlook is briefly
    /// unresponsive (#139) the delete fails, the draft stays in the user's real Drafts folder, and
    /// the run reports green. It was found by looking, not by any test - one draft from a run at
    /// 01:46 was still sitting in the live mailbox. A cleanup that fails silently is the same defect
    /// as a scan that stops early and says nothing, applied to someone's actual mail.
    /// </para>
    ///
    /// <para>
    /// One retry first, because the observed failure is transient rather than a refusal. The
    /// deliberate trade-off: this asserts from a <c>finally</c>, so if the test body <i>also</i>
    /// threw, this exception replaces it and the original cause is lost. That is accepted because
    /// the failing case is the one that matters - a body that passed while leaving residue behind
    /// used to be invisible, and is now loud - and the message says plainly that cleanup was the
    /// problem, so a reader knows to look past it.
    /// </para>
    /// </summary>

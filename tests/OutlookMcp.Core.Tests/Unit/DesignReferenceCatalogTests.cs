...
using OutlookMcp.Core.Data;
using OutlookMcp.Core.Tests.Helpers;
using Xunit;
...
public class DesignReferenceCatalogTests
{
    ...
    {
        var tempRepoRoot = Path.Combine(Path.GetTempPath(), $"OutlookMcpEvalAssets_{Guid.NewGuid():N}");
        ...
        var previousAssetRepoRoot = Environment.GetEnvironmentVariable("OUTLOOKMCP_EVAL_ASSET_REPO_ROOT");
        var previousReferenceRoot = Environment.GetEnvironmentVariable("OUTLOOKMCP_REFERENCE_DATA_ROOT");

        try
        {
            Environment.SetEnvironmentVariable("OUTLOOKMCP_REFERENCE_DATA_ROOT", null);
            Environment.SetEnvironmentVariable("OUTLOOKMCP_EVAL_ASSET_REPO_ROOT", tempRepoRoot);
        ...
        {
            Environment.SetEnvironmentVariable("OUTLOOKMCP_EVAL_ASSET_REPO_ROOT", previousAssetRepoRoot);
            Environment.SetEnvironmentVariable("OUTLOOKMCP_REFERENCE_DATA_ROOT", previousReferenceRoot);
            ResetReferenceCatalogCache();

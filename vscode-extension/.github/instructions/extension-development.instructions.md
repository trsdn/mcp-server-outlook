...
> **Instructions for developing the OutlookMcp VS Code Extension**
...
**The extension uses**: `dotnet tool run mcp-outlook`

**NOT**: `dnx OutlookMcp.McpServer --yes` (this is incorrect)
...
# Actual command should be: dotnet tool run mcp-outlook
```
...
2. **Don't use dnx commands in documentation**
   - Extension uses `dotnet tool run mcp-outlook`
...
2. **Use correct command syntax** (`dotnet tool run mcp-outlook`)
...
3. **Correct command syntax** - `dotnet tool run mcp-outlook` (not dnx)
